VERS EQU '0 ' ; December 4, 2021 19:33 drm "Z67.ASM"
*************************************************************************
...
login:	call	get$ptr		; set dataptr & ix to current lun data
...
INIT$HARD:
	...
	lda	@scrcb+14
	sta	bbnk		; set disk bank = 0
	lhld	@scrcb+12	; use bios scratch buffer for magic sector
	shld	bdma
...
nxtdef	push	b		; MOVE PARTITION ADDRESS TABLE INTO DRIVER
	...
	mvi	a,DCTYPE	; GET L.U.N. SPECIFIC DATA FROM MAGIC SECTOR
	call	bufoff
...
DO$RW:	STA	CMBFR		; COMMAND BUFFER OP CODE
	...
	ldy	h,+13
	shld	bdma
	call	get$ptr		; Set dataptr and ix to current lun data
...
NXDIV:	SRAR	C
	...
	RARR	L
	DJNZ	NXDIV
NODIV:	ldy	a,+10		; GET REQUESTED SECTOR
	MOV	E,A
	MVI	D,0
	DAD	D		; ADD IT IN
	JRNC	NOCAR1
...
LUN:	DB	0		; CURRENT LUN  (Set when get$ptr is called)
