...
main(argc,argv) 		/* main entry point from CP/M */
	ushort argc;
	char *argv[];
	{
...
getfile(filename,drive) 		/* Setups and opens the bios file */
	char *filename,*drive;		/* when the file is not known */
...
startup(filename)			/* Setups and opens the bios file */
	char *filename; 		/* when it's known (in the command */
	{				/* line) */
	writeflg=FALSE;
	if(openbios(filename)==ERROR)
		{
...
mainmenu(filename)			/* Select submenu and call exec */
	char *filename; 		/* menu routine to do it */
	{
	short line,cond;
...
prtsignon()				/* prints signon message */
	{
	char *getterm();
...
prtmfix()				/* prints fixed parts of main menu */
	{
...
prtmvar()				/* prints variable parts of menu */
	{
	ushort i;
...
selmain()				/* select a submenu */
	{
	short inp,nline;

	if(mpmfile) nline=MPMNLNE+numchario+numdiskio;
	else nline=NLNE+numchario+numdiskio;
	if(bioscurflg) ++nline;
	do
	    {
	    curcol=0;
	    ...
	    inp=getkey();
	    putwin(4,"");
	    if(inp==DOWN) {if(++curline>=nline) curline=nline;}
	    else if(inp==HMCD) {curline=0;}
	    else if(inp==UP) {if(--curline<=0) curline=0;}
	    else if(inp!=CRCD) bell();
	    }
	while(inp!=CRCD);
	}

execmain(filename)		/* Execute a submenu */
	char *filename; 
	{	       
	if(mpmfile)    
		{		/* If MP/M file */
...
chainfil(altdrv,name,opt1,opt2)
	char *name,*opt1,*opt2; 	/* command to chain to */
	short altdrv;			/* drive in bios file name */
	{
	short drv,logdrv;
	ushort adr;
	char filename[20];
...
chkmbios()			/* checks if system is valid by checking for */
	{			/* 16 JMP's in a row */
	byte jump;
	short i,retcode;
