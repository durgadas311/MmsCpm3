...
initlptbl(drvtbl)		/* Set default entries in the log/phy table */
	DRVTABL *drvtbl;
	{
	short i,j,pdrv,ldrv;
...
prtlptbl(drvtbl)		/* print all variable portitions of menu */
	DRVTABL *drvtbl;
	{
	curoff();
	if(!mpmfile)
...
getlfld(drvtbl) 		/* move cursor to a field and if a non cntrl */
	DRVTABL *drvtbl;	/* type character is entered goto the field */
...
prtlmsg()				/* print window help messages */
	{
	if(oldcol==curcol)
		return;
	oldcol=curcol;
...
tempfld(drvtbl) 			/* Change temporary drive field */
	DRVTABL *drvtbl;
	{
	short inp;
...
schfld(drvtbl)				/* change drive search field */
	DRVTABL *drvtbl;
	{
	short inp,i,j;
...
chkdrvtbl(drvtbl)			/* checks temporary and file search */
	DRVTABL *drvtbl;		/* order fields to if one is assigned*/
	{				/* to an deleted drive. */
	short i,j;
...
prtsord(drvs)				/* print drive search order field */
	byte *drvs;
	{
	char *sordstr();
	short i,j;
...
prttdrv(tdrv)				/* print temporary drive field */
	byte tdrv;
	{
	short i;
...
prtpnum(lptbl)				/* print physical drive number */ 
	byte *lptbl;
	{
...
prtstr(lptbl)				/* print module text string */
	byte *lptbl;
	{
	ushort phynum,i;
...
prtdup(lptbl)				/* print or clear duplicate msg */
	byte *lptbl;
	{
	short i,j;
	byte duparr[MAXDRV];
