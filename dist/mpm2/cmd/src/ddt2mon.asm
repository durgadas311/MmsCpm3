	TITLE	'MP/M DEBUGGER (DEMON) 9/81'
;	MP/M II DEBUGGER VERSION 2.0
;
...
LOF	EQU	5
;
rawin	equ	3	;raw character in (no echo)
GETF	EQU	10	;FILL BUFFER FROM CONSOLE
CHKIO	EQU	11	;CHECK IO STATUS
ver	equ	12	;get mp/m version #
OPF	EQU	15	;DISK FILE OPEN
clf	equ	16	;close disk file
delf	equ	19	;delete disk file
RDF	EQU	20	;READ DISK FILE
wrf	equ	21	;write disk file
mkf	equ	22	;make disk file
DMAF	EQU	26	;SET DMA ADDRESS
;
pfnm	equ	152	; parse file name
sda	equ	154	; system data address
;
...
;
;	verify that MP/M is running
	mvi	c,ver
	call	trapad
	dcr	h	;H must = 01h for MP/M
	mvi	c,0
	jnz	trapad	;system reset if not MP/M

	mvi	c,sda
	call	trapad		; get system data page address
	inx	h
	inx	h
	mov	a,m		; get RSTNUM
	rlc
	rlc
	rlc
	ori	0c7h		; compute RSTIN
	sta	rstin

;  fill in brkpt vector entry in system data page

	mvi	l,0fch		; offset to mpm dseg base adr
	mov	b,h
	mov	e,m
	inx	h
	mov	d,m
	lxi	h,5		; offset to rlr in mpm dseg
	dad	d
	mov	e,m
	inx	h
	mov	d,m
	lxi	h,0fh		; offset to memseg byte in PD
	dad	d
	mov	a,m
	add	a		; double memseg index for word offset
	adi	30h		; offset to brkpt vctr in sys dat pg
	mov	l,a
	mov	h,b
	mvi	m,low(breaka)
	inx	h
...
;	CHECK FOR FILE NAME PASSED TO DEMON, AND LOAD IF PRESENT
	lhld	base
	mvi	l,FCB+FFN	;BLANK IF NO NAME PASSED
	mov	a,m
	CPI	' '
	JZ	rdone		; display NEXT and PC
;
;	PUSH A base address, AND READ
	lhld	base
	PUSH	H
	JMP	RINIT
;
...
;	DISASSEMBLER NOT PRESENT, SET BDOS JMP
	LXI	d,DEMON
	lhld	base
	mvi	l,BDOSE+1	;(RE)SET JMP ADDRESS
	mov	m,e
	inx	h
	mov	m,d
DISASMOK:
...
JMPTAB:	;JUMP TABLE TO SUBROUTINES
	...
	DW	bitsr	;B Bit set / reset for PRL bitmap
	DW	CERROR	;C
	...
	DW	TRACE	;T
	DW	UNTRACE	;U
	DW	value	;V Calculate # sectors for diskwrite
	DW	write	;W WRITE FILE
...
;
; Bit set / reset for PRL file bit map
bitsr:
	call	scanexp
	cpi	2
	jnz	cerror	;requires two params, address & 0/1
	call	getval
	push	h
	call	getval
	mvi	a,1
	...
	push	psw
	push	h
	mov	a,l	;A = low order byte of byte address
	ani	111b
	inr	a
	mov	b,a
	mvi	a,01h
bit0:
	rrc
	dcr	b
	jnz	bit0	;shift bit into proper position
	lhld	base
	lxi	b,0101h
	dad	b
	mov	c,m
	inx	h
	mov	b,m	;bc = # bytes of code, offset to start of bit map
	lxi	d,00feh
	dad	d
	dad	b
	pop	d
	...
	dcr	d
	mvi	c,3
bit1:			;shift addr right 3 to get bit map byte offset
	mov	a,d
	ora	a
	rar
	mov	d,a
	...
	dcr	c
	jnz	bit1
	dad	d	;hl = address of bit map byte
	call	chkadr	;check for ddt mon overwrite
	pop	psw
	mov	a,b
	jz	bit2	;jump if bit to be reset
	ora	m
	mov	m,a
	jmp	start
bit2:			; turn the bit off
	cma
	ana	m
	mov	m,a	;update bit map
	jmp	start
;
...
DISP1:	;0 OR 1 EXPN, DISPLAY HALF SCREEN
	...
	DAD	D
	jnc	disp2
	lxi	h,0ffffh ;avoid wrap-around display
DISP2:	SHLD	DISMAX
...
FILL0:	CALL	BCDE	;END OF FILL?
	...
	mov	a,b
	ora	c
	jz	start	;avoid wrap-around memory
	JMP	FILL0
;
...
;	FILL FCB AT 5CH
	...
	jnz	cerror
	lhld	base
	mvi	l,fcb+fcr ;CLEAR CURRENT RECORD
	mov	m,a
	mvi	a,0ffh
	sta	rdwrok	;ok to read/write
	JMP	START
;
pfcb:			; parse filename control block
	dw	$-$	;    adr of ASCII filename string
	dw	$-$	;    adr of FCB
;
;	MOVE MEMORY
MOVE:
...
MOVE0:	;HAS B,C PASSED D,E?
	...
	mov	a,h
	ora	l
	jz	start	;avoid wrap-around move
...
CHKDIS:	;CHECK FOR DISASSM PRESENT
	...
	RET
;
chkadr:	;check for ddt mon overwrite
	;this routine assumes that low(demon) <> 0
	push	psw
	mvi	a,low(demon)-1
	sub	l
	mvi	a,high(demon)
	sbb	h
	jc	cerror	; attempting to overwrite ddt
	pop	psw
	ret
;
; Page relocation of a loaded PRL file
npreloc:
	lhld	base
	push	h
	lxi	d,0101h
	...
	mov	c,m
	inx	h
	mov	b,m	;bc = # bytes to relocate
	push	b
	lxi	d,00feh
	dad	d
;	hl = base of code area to relocate
;	bc = length of code segment
pgmove:
	mov	a,b	;bc=0?
	ora	c
	jz	pgrel
	...
	dcr	h
	call	chkadr	;check for possible ddt mon overwrite
	mov	m,a	;move it down 100h bytes
	inr	h
	inx	h
	jmp	pgmove
;
pgrel:	;storage moved, ready for relocation
;	hl addresses beginning of the bit map for relocation
	...
	inr	d	;de = base of code to relocate
pgrel0:
	mov	a,b	;bc=0?
	ora	c
	jz	start
...
pgrel1:
	...
	jnc	pgrel2	;skip relocation if cy=0
;
...
READ:
	lda	rdwrok
	ora	a
	jz	cerror	;read must be preceeded with a I cmnd
	CALL	SCANEXP
	LXI	H,0
	JZ	READN
...
readn:
	xra	a
	sta	rdwrok	;clear read/write flag
	xchg
	lhld	base
	xchg
	dad	d
	...
	mvi	e,80h
	mvi	c,dmaf
	call	trapad	;reset the DMA address to tbuff
RINIT:
...
;	COM FILE, LOAD WITH OFFSET GIVEN BY PUSHED REGISTER H
	POP	H
	inr	h	;BASE OF TRANSIENT AREA offset
...
LCOM1:	LDAX	D	;LOAD NEXT BYTE
	INX	D
	call	chkadr	;test for ddt mon overwrite
...
;
; Calculate # of sectors given NEXT
value:
	call	scanexp
	jz	rdone	; display NEXT and PC
	dcr	a
	jnz	cerror
	call	getval	;get the value of NEXT
	lxi	d,007fh
	dad	d	;round up to next whole sector
	xchg
	lhld	base
	inr	h	;offset by base+100h
	mov	a,e
	sub	l
	mov	l,a
	mov	a,d
	sbb	h
	mov	h,a
	jc	cerror	;NEXT must be above base+100h
	xra	a	;divide by 80h
	mov	a,l
	ral
	mov	a,h
	...
	mov	h,a
	call	crlf
	call	paddr	;display result
	jmp	start
;
; Write contents of memory to disk
write:
	lda	rdwrok
	ora	a
	jz	cerror	;must preceed write with an I cmnd
	call	scanexp
	jz	cerror	;requires # 128 byte sectors to write
	dcr	a
	jnz	cerror	;only one parameter allowed
	sta	rdwrok	;clear read/write flag
	call	getval
	inx	h	;pre-increment
	push	h	;save # of sectors to write
	lhld	base
	mvi	l,dfcb
	xchg
	push	d	;DE = FCB address
	mvi	c,delf
	call	trapad	;delete the file
	pop	d
	push	d
	mvi	c,mkf
	call	trapad	;make the file
	inr	a
	jz	cerror	;no directory room available
	pop	d
	push	d
	mvi	c,opf
	call	trapad	;open the file
	inr	a
	jz	cerror
	lhld	base
	inr	h	;HL = base of area to write from
wr0:
	pop	d
	pop	b
	dcx	b
	...
	push	d
	mvi	e,dbf
	mvi	b,80h	;B = buffer size
wr1:
	mov	a,m	;copy from memory to the buffer
	stax	d
	inx	d
	inx	h
	dcr	b
...
GETBUFF:	;FILL COMMAND BUFFER AND SET POINTERS
	; intialize the command buffer to nulls
	lxi	d,combuf+csize-1
	mvi	c,csize
	xra	a
getb0:
	stax	d
...
;	push	psw
;	mvi	c,rawin
;	cnz	trapad	;remove char from bfr if break
;	pop	psw
...
CLRTRACE:	;CLEAR THE TRACE FLAG
	...
	RET
;
rstin:	db	1	; restart instruction
...
rdwrok:	db	0	;read/write ok flag (i.e. I done)
;	DS	SSIZE	;STACK AREA
...
;
base:	NOP		;FOR RELOCATION BOUNDARY
	nop		; will contain mem seg base page
	END
